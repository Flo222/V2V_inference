...
def _is_v2xreal_dataset(dataset):
    """Return True for V2X-Real dataset classes."""
    name = dataset.__class__.__name__.lower()
    module = dataset.__class__.__module__.lower()
    return "v2xreal" in name or "v2xreal" in module


def _post_process_compatible(batch_data, output_dict, dataset):
    """
    OPV2V post_process returns:
        pred_box_tensor, pred_score, gt_box_tensor

    V2X-Real post_process returns:
        pred_box_tensor, pred_score, gt_box_tensor, gt_label_tensor

    Keep OPV2V behavior unchanged, but allow V2X-Real inference to receive
    the class labels required for multi-class evaluation.
    """
    ret = dataset.post_process(batch_data, output_dict)

    if isinstance(ret, (tuple, list)) and len(ret) == 4:
        if _is_v2xreal_dataset(dataset):
            return ret
        return ret[:3]
...
def inference_late_fusion(batch_data, model, dataset):
    ...
    return _post_process_compatible(batch_data, output_dict, dataset)
