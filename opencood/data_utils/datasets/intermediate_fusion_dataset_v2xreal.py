...
class IntermediateFusionDatasetV2XReal(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            # Link-level Markov channel state produced by
            # BaseDataset.retrieve_base_data(). These are consumed by ARCE to
            # select good / medium / bad profiles per ego->CAV link.
        ...
        channel_delay_slots = channel_delay_slots + \
            (self.max_cav - len(channel_delay_slots)) * [0]
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose):
        ...
        # V2X-Real boxes are 8-dim center boxes:
        # [x, y, z, h, w, l, yaw, class_id].
        # box_utils.mask_boxes_outside_range_numpy expects 7-dim center boxes
        # or 8-corner boxes, so use only the first 7 dims for range checking,
        # then keep the original 8-dim boxes after masking.
        object_bbx_center_valid = object_bbx_center[object_bbx_mask == 1]

        if object_bbx_center_valid.shape[-1] > 7:
            _, range_mask = box_utils.mask_boxes_outside_range_numpy(
                object_bbx_center_valid[:, :7],
    ...
    def collate_batch_train(self, batch):
        ...
        channel_delay_slots = torch.from_numpy(
            np.array(channel_delay_slots, dtype=np.int64)
        )
        spatial_correction_matrix_list = \
