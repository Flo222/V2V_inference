...
except ImportError:  # optional FLOP-profiling dependency
    def profile(*args, **kwargs):
        raise ImportError("thop is required only when CoopDiff FLOP profiling is requested")
from torch import nn
import math
from torch.autograd import Variable

from opencood.models.baselines.coopdiff.diffuser.object_fusion import ConditionGuidedCrossAttention
from opencood.models.baselines.coopdiff.diffuser.selection import CollaborativeFusion, GatedFusion, CollaborativeFusion_DeCooper
# from mmcv.runner import auto_fp16, force_fp32
from opencood.models.baselines.coopdiff.diffuser.utils import cosine_beta_schedule, extract, replace_nan, check_nan, SinusoidalPositionEmbeddings
from opencood.models.baselines.coopdiff.diffuser.fuser_blocks import AttentionBlock, SeparableConvBlock, DoubleInputSequential
...
def get_next_sample_idx(counter_file='/home/my/桌面/chengong/DSRC/opencood/logs/diffuser/pics/counter.txt'):
...
def get_save_path(sample_idx, suffix):
    return f"/home/my/桌面/chengong/DSRC/opencood/logs/diffuser/pics/{sample_idx}_{suffix}.png"
...
class Diffuser(nn.Module):  # input size: batch, C, width, length [1,64,400,600]
    def __init__(self,
        ...
        ################################
        self.down1 = nn.Sequential(nn.Conv2d(self.in_planes, mid_planes[0], 3, padding=1, stride=strides[0]),
                                             nn.BatchNorm2d(mid_planes[0], momentum=0.1, eps=1e-3),nn.SiLU(),)
        self.down2 = nn.Sequential(nn.Conv2d(mid_planes[0], mid_planes[1], 3, padding=1, stride=strides[1]),
                                             nn.BatchNorm2d(mid_planes[1], momentum=0.1, eps=1e-3),nn.SiLU(),)
        self.down3 = nn.Sequential(nn.Conv2d(mid_planes[1], mid_planes[2], 3, padding=1, stride=strides[1]),
                                             nn.BatchNorm2d(mid_planes[2], momentum=0.1, eps=1e-3),nn.SiLU(),)

        # self.localGenerate1 = CollaborativeFusion(mid_planes[0])
        # self.localGenerate2 = CollaborativeFusion(mid_planes[1])
        # self.localGenerate3 = CollaborativeFusion(mid_planes[2])
        self.localGenerate4 = CollaborativeFusion_DeCooper(mid_planes[1])

        self.deconv1_sup = nn.Sequential(
            nn.ConvTranspose2d(mid_planes[0],num_upsample_filters[0],int(upsample_strides[0]),stride=int(upsample_strides[0]), bias=False) if upsample_strides[0] >= 1 else
            nn.Conv2d(mid_planes[0],num_upsample_filters[0],kernel_size=int(1 / upsample_strides[0]),stride=int(1 / upsample_strides[0]),bias=False),
            nn.BatchNorm2d(num_upsample_filters[0], eps=1e-3, momentum=0.1),
            self.act
        )
        self.deconv2_sup = nn.Sequential(
            nn.ConvTranspose2d(mid_planes[1],num_upsample_filters[1],int(upsample_strides[1]),stride=int(upsample_strides[1]), bias=False) if upsample_strides[1] >= 1 else
            nn.Conv2d(mid_planes[1],num_upsample_filters[1],kernel_size=int(1 / upsample_strides[1]),stride=int(1 / upsample_strides[1]),bias=False),
            nn.BatchNorm2d(num_upsample_filters[1], eps=1e-3, momentum=0.1),
            self.act
        )
        self.deconv3_sup = nn.Sequential(
            nn.ConvTranspose2d(mid_planes[2],num_upsample_filters[2],int(upsample_strides[2]),stride=int(upsample_strides[2]), bias=False) if upsample_strides[2] >= 1 else
            nn.Conv2d(mid_planes[2],num_upsample_filters[2],kernel_size=int(1 / upsample_strides[2]),stride=int(1 / upsample_strides[2]),bias=False),
            nn.BatchNorm2d(num_upsample_filters[2], eps=1e-3, momentum=0.1),
            self.act
        )

        # 门控融合 可以交互起来，修改！（选择性特征和全局特征的融合）
        #               效果不好可能是因为开始时候的阈值选择纯噪声，影响结果了
        # self.gate1 = GatedFusion(mid_planes[1])
        # self.gate2 = GatedFusion(mid_planes[1])
        # self.gate3 = GatedFusion(mid_planes[1])

        # 门控融合*，选择性特征条件融合
        self.gate1 = ConditionGuidedCrossAttention(num_upsample_filters[1], max_position_embeddings=24*88)
        self.gate2 = ConditionGuidedCrossAttention(num_upsample_filters[1], max_position_embeddings=24*88)
        self.gate3 = ConditionGuidedCrossAttention(num_upsample_filters[1], max_position_embeddings=24*88)
        ...
        self.loss_fn = FocalLoss(gamma=2, alpha=0.75)
        self.focal_head = nn.Conv2d(num_upsample_filters[1]*3, 2, kernel_size=1)
    ...
    def _forward_fast_attention(self, p1, p2, p3):
        ...
        # 处理 p2、p2_up 和 p1_out 并生成 p2_out
    ...
    def forward(self, x_start, x_condition, fea_stu_others=None, mask=None, t=None):
        ...
        if self.training:
            ...
            print('t:',t.item(),end=' ')
            # split_ratio = 0.5 + 0.3 * (current_epoch / 30) #逐渐加大高噪声注入
            # high_noise = torch.rand(1, device=self.device) < split_ratio
            #
            # t_high_start = int(self.num_timesteps * 0.8)
            # t_high = torch.randint(t_high_start, self.num_timesteps, (1,), device=self.device).long()
            # t_low = torch.randint(0, t_high_start, (1,), device=self.device).long()
            # t = torch.where(high_noise, t_high, t_low)
        else:
            t = torch.tensor([999], device=self.device).long()


        condition = x_condition #条件特征
        condition_others = fea_stu_others

        x_t = self.q_sample(x_clean, t, noise=noise) #加噪
        ...
        if self.training:
            x_pred, noise_pred, label_loss = self.model_prediction(x_t, condition, condition_others, t, mask)
            ...
            print()
            #print('label loss:', label_loss.item(),' | diffuse_loss:',diffuse_loss.item() )
            diffuse_loss = diffuse_loss + label_loss # 乘0.1是因为后面要整体乘10，但他这个loss偏大了

        else:
            if self.sampler == 'ddim':
                x_pred, noise_pred, intermediate_feats = self.ddim_sampling(x_t, condition, condition_others)
            elif self.sampler in ['dpmsolver', 'dpmsolver++', "sde-dpmsolver", "sde-dpmsolver++", 'deis']:
                x_pred, noise_pred, intermediate_feats = self.dpm_deis_sampling(x_t, condition, condition_others)
            else:
                raise NotImplementedError
    ...
    def model_prediction(self, x_t, x_start, x_start_others, ts, mask=None):
        ########## Step 1: 时间嵌入 ###########
        t = self.time_emb(ts)
        # Ver 1
        c = x_start + t.unsqueeze(-1).unsqueeze(-1)                 # 综合特征/(attn实现的好像是只选用[0]的自车特征)
        c_others = x_start_others + t.unsqueeze(-1).unsqueeze(-1)   # 它车特征
        ...
        # torch.Size([2, 128, 24, 88]) torch.Size([2, 128, 12, 44]) torch.Size([2, 128, 6, 22])

        ########## Step 3: 特征融合 ###########
        if self.attention:
            # (p1,p2,p3): (N mid_planes[1] H W) (N mid_planes[1] H/2 W/2) (N mid_planes[1] H/4 W/4)
            # (p1_out,p2_out,p3_out)与 (p1,p2,p3) 一致
        ...
        if self.training:
            ...
            # 对标签裁剪，对每个尺度的特征都进行选择  训练用真标签，此外额外训练一个标签选择器测试使用。使用deformatt生成选择区域？
            c_others1 = self.down1(c_others)
            c_others2 = self.down2(c_others1)
            c_others3 = self.down3(c_others2)

            c_all = [c1,c2,c3]
            c_others_all = [c_others1, c_others2, c_others3]

            p1_sup, p2_sup, p3_sup = self.localGenerate4(c_all, c_others_all)

        else:
            # # 对标签裁剪，对每个尺度的特征都进行选择  训练用真标签，此外额外训练一个标签选择器测试使用。使用deformatt生成选择区域？
            # c_others = self.down1(c_others)
            # p1_sup = self.localGenerate1(c1, c_others)
            #
            # c_others = self.down2(c_others)
            # p2_sup = self.localGenerate2(c2, c_others)
            #
            # c_others = self.down3(c_others)
            # p3_sup = self.localGenerate3(c3, c_others)
        ...
        up1_sup = self.deconv1_sup(p1_sup)
        up2_sup = self.deconv2_sup(p2_sup)
        up3_sup = self.deconv3_sup(p3_sup)

        sup_all = self.focal_head(torch.cat([up1_sup, up2_sup, up3_sup], dim=1))


        if self.training and mask is not None:
            mask = mask.squeeze(1).long()
            label_loss = self.loss_fn(sup_all, mask)
        else:
            label_loss = 0
        ...
        # # 额外补充的 两个解码器的融合
        up1 = self.gate1(up1, up1_sup)
        up2 = self.gate2(up2, up2_sup)
        up3 = self.gate3(up3, up3_sup)
        ...
        if self.objective == 'pred_x0':
            ...
            x_pred = self.predict_start_from_noise(x_start,
                                                   ...
                                                   pred_noise)
        if self.training:
            return x_pred, pred_noise, label_loss
        else:
            return x_pred, pred_noise

    @torch.no_grad()
    def ddim_sampling(self, x_t, x_start, x_start_others):

        # # # 获取本次样本编号
        # sample_idx = get_next_sample_idx()
        # x_start_processed = process_tensor(x_start)
        # show_heatmaps(x_start_processed[np.newaxis, np.newaxis, ...],
        #               path=get_save_path(sample_idx, "x_start"))
        ...
        for i, (time, time_next) in enumerate(time_pairs):
            # print(f"DDIM sampling timestep {i+1}/{len(time_pairs)}: {time} -> {time_next}", end='\r')
            ts = torch.tensor([time], device=x_start.device, dtype=torch.long)
            x_t, pred_noise = self.model_prediction(x_t, x_start, x_start_others, ts)
            if time_next < 0:
                continue
            ...
            # 保存 x_t
            # x_t_processed = process_tensor(x_t)
            # show_heatmaps(x_t_processed[np.newaxis, np.newaxis, ...],
            #               path=get_save_path(sample_idx, f"x_t{i + 1}"))
    ...
    def dpm_deis_sampling(self, x_t, x_start, x_start_others, return_intermediate = False):
        ...
        for step_index, (time, time_next) in enumerate(time_pairs):
            ...
            # x_t_processed = process_tensor(x_t)
            # show_heatmaps(x_t_processed[np.newaxis, np.newaxis, ...],
            #               path=get_save_path(sample_idx, f"x_t{step_index + 1}"))
...
def calculate_gpu_usage():
    ...
    # 计算占比
    usage_ratio = (allocated_memory / total_memory) * 100
    max_usage_ratio = (max_allocated_memory / total_memory) * 100
    return {
        "total_memory_MB": total_memory,
        "allocated_memory_MB": allocated_memory,
        "max_allocated_memory_MB": max_allocated_memory,
...
if __name__ == "__main__":
    ...
    others = torch.randn(batch_size, 256, input_height, input_width, requires_grad=True).to(device)
    x = cam
    mask = torch.randint(0, 2, (batch_size, 1, input_height, input_width), dtype=torch.float32).to(device)

    x_pred, diffuse_loss, _ = model(x, x, others, mask)
    ...
    # try:
    #     # Run a forward pass
    #     flops, params = profile(model, inputs=(x, x), verbose=True)
    #
    #     # Convert FLOPs to GFLOPs
    #     gflops = flops / 1e9
    #     print(f"Total estimated FLOPs: {gflops:.2f} GFLOPs")
    #
    ...
    #     memory_params = total_params * 4 / (1024 ** 2)  # 4 bytes per float32 parameter
    #     print(f"Total memory for model parameters: {memory_params:.2f} MB")
    #
    #     #
    #     # # 打印第一层模块占用的memory_params
    #     # print("\nMemory usage for first - level modules:")
    #     # for name, module in model.named_children():
    #     #     module_params = sum(p.numel() for p in module.parameters())
    #     #     module_memory = module_params * 4 / (1024 ** 2)
    #     #     print(f"Module: {name}, Memory: {module_memory:.2f} MB")
    #     #
    #
    #     print('---------')
    #
    ...
    #     # 前向传播
    #     usage_before = calculate_gpu_usage()
    #
    #     out = model(x, x)
    #     usage_after = calculate_gpu_usage()
    #
    #     print(f"总显存: {usage_after['total_memory_MB']:.2f} MB")
    #     print(f"前向传播前显存占用: {usage_before['allocated_memory_MB']:.2f} MB")
    #     print(f"前向传播后显存占用: {usage_after['allocated_memory_MB']:.2f} MB")
    #     print(f"峰值显存占用: {usage_after['max_allocated_memory_MB']:.2f} MB")
    #     print(f"当前显存占比: {usage_after['usage_ratio']:.2f}%")
    #     print(f"峰值显存占比: {usage_after['max_usage_ratio']:.2f}%")
    #
    #
    #
