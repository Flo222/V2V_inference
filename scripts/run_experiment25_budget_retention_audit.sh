...
OUT_ROOT="${OUT_ROOT:-audit_runs/experiment25_budget_retention}"
...
print("Experiment-2.5 fixed total frame budget: %.3f MB/frame (%g Mbps, %g ms)" % (
    frame_bytes / 1_000_000.0, mbps, window_ms
))
print("This experiment does not change packet selection. It only maps selected source packets back to feature channels/spatial positions.")
PY
...
arce["fixed_action"] = {
    ...
    "cache_enabled": 0,
    "fec_type": "none",
    "action_id": "experiment25_%s_rho0_cache0" % mode,
}
...
arce["compression_audit"] = {
    "enabled": True,
    "strict": True,
    "experiment_name": "experiment25_budget_retention_layout",
    "output_dir": os.path.abspath(audit_dir),
    "file_name": "budget_retention_audit.jsonl",
...
PY
  ...
  PYTHONUNBUFFERED=1 python opencood/tools/inference_arce.py \
    ...
    --comm_prefix "experiment25_${mode}" \
    --save_eval_json \
...
python scripts/summarize_experiment25_budget_retention_audit.py \
  --root "$OUT_ROOT" \
  --modes $QUANT_MODES \
  --strict

python scripts/visualize_experiment25_budget_retention.py \
  --root "$OUT_ROOT" \
  --modes $QUANT_MODES || true

echo
echo "Experiment 2.5 finished: $OUT_ROOT"
