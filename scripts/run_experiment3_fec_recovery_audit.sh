...
OUT_ROOT="${OUT_ROOT:-audit_runs/experiment3_fec_recovery}"
MAX_SAMPLES="${MAX_SAMPLES:-200}"
NUM_WORKERS="${NUM_WORKERS:-0}"
SEED="${SEED:-2026}"
SAVE_TENSORS="${SAVE_TENSORS:-1}"
SAVE_FIRST_N_LINKS="${SAVE_FIRST_N_LINKS:-12}"
QUANT_MODE="${QUANT_MODE:-int8}"
PLRS="${PLRS:-0.20 0.35}"
RHOS="${RHOS:-0 0.10 0.25 0.60}"
SYSTEM_BUDGET_MBPS="${SYSTEM_BUDGET_MBPS:-10000}"
...
case "$QUANT_MODE" in
  fp16|int8|int4) ;;
  *) echo "Experiment 3 expects an integer/low-bit mode; use int8 by default. Got: $QUANT_MODE" >&2; exit 1 ;;
esac

condition_name() {
  python - "$1" "$2" <<'PY'
import sys
plr = float(sys.argv[1]); rho = float(sys.argv[2])
def t(x): return ("%.2f" % x).replace(".", "p")
print("plr_%s_rho_%s" % (t(plr), t(rho)))
PY
}
...
make_runtime_dir() {
  ...
  local rho="$2"
  local condition
  condition="$(condition_name "$plr" "$rho")"
  ...
  python - "$MODEL_DIR/config.yaml" "$runtime_dir/config.yaml" "$QUANT_MODE" \
...
arce.update({"enabled": True, "mode": "fixed", "policy": "fixed", "link_scope": "non_ego", "keep_tensor_results": False, "seed": seed})
...
arce["fixed_action"] = {
    ...
    "fec_type": fec_type,
    "decode_overhead": 0.0,
    "action_id": "experiment3_%s_plr%.2f_rho%.2f" % (quant_mode, plr, rho),
}
...
for state in ("good", "medium", "bad"):
    ...
    p.update({"bandwidth_mbps": budget_mbps, "loss_rate": plr, "plr": plr, "delay_ms": 0.0})
    profiles[state] = p
...
# Disable prior audits for speed and enable the Experiment-3 read-only audit.
arce["compression_audit"] = {"enabled": False}
arce["fec_recovery_audit"] = {
    "enabled": True,
    "strict": True,
    "experiment_name": "experiment3_pure_fec_recovery",
...
PY
  ...
  echo
  echo "===== Experiment 3: PLR=$plr rho=$rho quant=$QUANT_MODE ====="
  echo "Sufficient frame budget: $SYSTEM_BUDGET_MBPS Mbps x $TX_WINDOW_MS ms"
  ...
  PYTHONUNBUFFERED=1 python opencood/tools/inference_arce.py \
    ...
    --comm_prefix "experiment3_${condition}" \
    --save_eval_json \
...
for plr in $PLRS; do
  for rho in $RHOS; do
    make_runtime_dir "$plr" "$rho"
  done
done

python scripts/summarize_experiment3_fec_recovery_audit.py \
  --root "$OUT_ROOT" --plrs $PLRS --rhos $RHOS --strict

echo
echo "Experiment 3 finished: $OUT_ROOT"
