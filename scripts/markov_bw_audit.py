...
# -*- coding: utf-8 -*-
"""
Measure actual Markov-channel transmitted bytes for one collaborative
perception baseline.
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Audit actual Markov transmitted MB/frame for one baseline."
    )
    ...
    with torch.no_grad():
        for sample_index, batch_data in progress:
            ...
            try:
                ...
                if not records:
                    for structure_name, structure in latest_model_structures(
                        model, output
                    ):
