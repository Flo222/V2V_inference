...
3. 用 `sample_packet_loss()` 按当前 profile 的 `packet_loss_rate` 产生包级 Bernoulli 丢失掩码。
...
它不应再在 YAML 或运行时创建自己的 Markov 链、带宽、丢包率或时延参数。
...
| CoopDiff | `coopdiff_markov_feature_channel.py` | 多尺度特征组织、扩散模型接口 | ARCE byte-stream 包化、状态、带宽、丢包、时延 |
...
CoopDiff 的多个尺度会先拼接为一个逻辑字节流，再通过 ARCE 的
`ByteStreamPacketizer` 形成固定长度包；带宽裁剪和丢包后，仍由同一
packetizer 还原字节流并按原始尺度 shape/dtype 写回。旧的 cell-major
序列化、cell 选择和 cell-to-packet 映射已移除。
